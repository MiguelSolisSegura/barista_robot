...
def generate_launch_description():
    ...
    world_file_arg = DeclareLaunchArgument(
        ...
        default_value=[get_package_share_directory(
            description_package_name), '/worlds/base.world'],
   ...
   # Include the Gazebo launch file with the modified launch arguments
    ...
    rsp_robot1 = Node(
        ...
        parameters=[{'frame_prefix': robot_name_1+'/', 'use_sim_time': use_sim_time,
                     'robot_description': Command(['xacro ', robot_desc_path, ' robot_name:=', robot_name_1, ' chassis_red:=false'])}],
        output="screen"
    )
    ...
    rsp_robot2 = Node(
        ...
        parameters=[{'frame_prefix': robot_name_2+'/', 'use_sim_time': use_sim_time,
                     'robot_description': Command(['xacro ', robot_desc_path, ' robot_name:=', robot_name_2, ' chassis_red:=true'])}],
        output="screen"
    )
